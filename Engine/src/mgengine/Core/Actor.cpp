...
Actor::Actor(World* pWorld,std::string pName, glm::vec3 pPosition, btCollisionShape* pCollider, float pMass)
	: GameObject(pName, pPosition), _mass(pMass), _world (pWorld)
{
...
void Actor::initRigidBody(btCollisionShape* pCollider) {
	...
	_world->physicsManager->AddCollisionActor(_rigidBody);
	//_rigidBody->setCollisionFlags(_rigidBody->getCollisionFlags() | btCollisionObject::CF_CUSTOM_MATERIAL_CALLBACK);
	//_rigidBody->setCollisionFlags(btCollisionObject::CF_NO_CONTACT_RESPONSE);
}
